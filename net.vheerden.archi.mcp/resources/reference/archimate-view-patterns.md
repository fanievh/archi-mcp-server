...
This MCP server exposes 70 tools for ArchiMate model manipulation. Model size impacts reliability:
...
4. **Nest a part inside its parent — do NOT draw the structural relationship.** When an element is part of another (`CompositionRelationship` / `AggregationRelationship`), or an actor/role/collaboration **performs** it (`AssignmentRelationship` from the team/role/collaboration to the behaviour it performs), or an owning `ApplicationCollaboration` **aggregates** it (an `AggregationRelationship` — ArchiMate permits no Assignment into an `ApplicationComponent`, so owning a component is never an Assignment; a business actor or role gets an `AssociationRelationship`, not an Aggregation), show that by **visual containment** — `add-to-view` / `add-group-to-view` with `parentViewObjectId` — and **exclude that relationship type from the `auto-connect-view` filter**. Drawing a Composition, Aggregation, or Assignment connector between a parent and its own nested part is an **error**: containment already states it, and the duplicate connector clutters the view. Caveat (Wierda): nesting is *ambiguous in general* — a bare box-in-box does not reveal which structural relationship it implies — so for a cross-structure relationship whose *type* the reader must see, keep the explicit connector. The parent→part case is **not** ambiguous: nest it, never draw it (see "Adding Connections").
...
6. Repeat until pass-throughs are eliminated. Note: edge crossings (connections crossing each other, not through elements) are often structurally unavoidable in dense many-to-many views — the severity-tiered rating caps their impact at "good" (Tier 3R) so they do not mask structural quality. Non-orthogonal terminals (diagonal source/target entries on connections) sit in Tier 2R, which caps their impact at "fair" — a heavier band than crossings, not a lighter one. Below ~20° they are typically invisible to the human eye, so **do not iterate on them once the view is otherwise clean**; read `overallExcludingAcceptedCosmetics` to tell a terminals-only "fair" apart from one carrying a real defect. Density-aware thresholds mean wider element spacing is the fix, not re-routing.
...
**When a label has nowhere to go at all:** on a dense flow view a few labels collide at *every* position, and moving them just relocates the problem. `auto-route-connections` and `auto-layout-and-route` accept `labelPolicy: "auto-hide-on-collision"`, which hides exactly those labels and leaves every placeable label visible. On `auto-layout-and-route` it needs a routing pass, so it is honoured in `mode: "grouped"` and in flat mode when `targetRating` is set; flat mode *without* `targetRating` uses ELK's own edge routes, runs no label optimizer, and **rejects** the parameter rather than silently ignoring it. The response lists each hidden label individually under `hiddenLabels` (`connectionId` + `reason`) — never just a count — so you can restore any of them with `update-view-connection(showLabel: true)`. It is **opt-in**: omit the parameter and label visibility is untouched. A label you hid yourself is never overridden and never reported as a policy hide.
...
> **Large hubs** = any element on the view with **> 6 connections**. This is a distinct, higher cut than the `≥ 5 connections` hub *candidate* threshold in § 1 above — a hub candidate is not automatically a large hub. The hub-aware tier values account for the corridor space that formula-resized hubs consume — without them, the heuristic UNDERSHOOTS post-hub-resize and coincSeg residuals persist. The three spacing tools (`apply-element-spacing-recommendations`, `apply-group-spacing-recommendations` and `apply-spacing-recommendations`) automatically detect large hubs and apply the hub-aware tier; if you call `adjust-view-spacing` directly with explicit `interElementDelta` or `interGroupDelta`, you must factor in hub size manually. The unconnected column (40/40/60) is hub-agnostic — there are no inter-group corridors to widen.
...
> **Footnote — control-loop semantics inside the convenience tools.** The three convenience tools (`apply-element-spacing-recommendations`, `apply-group-spacing-recommendations`, `apply-spacing-recommendations`) embed an **observe → decide → density-aware-terminate control loop**. Per iteration they take a small `+10 px`-per-step monotone increment (a larger step when escalating), re-run `assess-layout`, and classify on a 2×2 of *aggregate-trend* × *spacing-regime-position*: still climbing → **CONTINUE**; stalled and below the prescribed ~100–124 px / fan-out-sized-hub regime → **ESCALATE** (large steps toward the ~112 px mid-band plus a one-shot hub-resize); stalled and already in-regime → **PASS-HONEST** (more spacing cannot help — stop and report). A degrading step is reverted, but only as the loop's own six-input step scalar measures it — that scalar reads element overlaps as a single bit and does not read cousin overlaps, off-canvas placement or `overallRating` at all, so **these tools can return a view rated worse than the one they were handed**. They now compare the `before` and `after` `assess-layout` snapshots on the overall rating and report a regression as a `SPACING_RATING_REGRESSED` structured warning (with a `nextSteps` line) naming both ratings and every metric that moved; the spacing stays applied and the caller decides. On a queued (batch) call the comparison is structurally unavailable and `nextSteps` says so. All accepted iterations wrap in one compound command (one call = one undo entry), so one `undo` reverses a whole run. The +80px element / +100px inter-group constants are **per-iteration step caps**, not per-call total clamps. `iterationBudget` defaults to 5 (single-arm) / 8 (composed), range [1, 20]; `dryRun: true` previews without mutating. `terminationReason` names exactly one of the branches below — the count is deliberately not stated here, because only the shipped routing-preconditions checklist has a test keeping a number honest, and this surface's number had been wrong: (a) `goal_reached_at_iteration_N` — **not reachable through these three tools** (the loop's goal predicate defaults to false and the spacing callback factory never overrides it, so no spacing call terminates here); (b) `budget_exhausted_after_N_iterations`; (c) `aggregate_threshold_regressed_at_iteration_N_reverted_to_iteration_M`; (d) `structural_no_change_<reason>`; (e) `heuristic_already_met_no_change`; (f) `dry_run_recommendation_not_applied`; (g) `iteration_apply_failed_at_iteration_N_reverted_after_M_accepted_iterations` (a contained mutation threw mid-application; best-effort rollback applied, prior accepted iterations preserved); (h) `density_floor_reflow_required` — **PASS-HONEST**: a sound pre-routing infeasibility certificate fired (average spacing already in the 100–124 px band and the hub sized for its connection count, but aggregate quality stalled). The tool stops without degrading the view, returns a `densityFloorDiagnosis` string, and **offers** a structural reflow as an explicit user-consentable next step — it never auto-reflows. On `density_floor_reflow_required`, surface the diagnosis and reflow offer to the user and wait for consent; do not loop the spacing tools. Two further **pre-loop** guards exist and are returned before the loop is entered at all: (i) `reroute_degraded_input_baseline` — the tool's internal pre-loop reroute pass scored a strictly lower aggregate than the bare input baseline, so the reroute would have degraded the input; the bare input is returned untouched (`iterationCount = 0`, `appliedDeltas = []`, no mutation); and (j) `density_precondition_infeasible_reflow_required` — a sound one-sided infeasibility certificate (`idealUniformAvg = sqrt(unionArea/N) − avgBox < 100`, zero false positives by construction) fired on the *input*, so the view is returned untouched with a `densityFloorDiagnosis` and a consent-gated reflow offer. It is honestly distinct from the in-loop `density_floor_reflow_required`: *the input precondition is infeasible and the loop was never entered*, not *the loop reached an in-regime density floor*. **`N` in the three `_iterations` tokens counts accepted COMMANDS, not iterations**: an escalate iteration pushes two commands (a one-shot hub-resize plus that iteration's spacing command), so `N` exceeds `iterationCount` by the number of hub resizes. `budget_exhausted_after_3_iterations` beside `iterationCount: 2` is consistent — read `iterationCount` when you want iterations.
...
|--------|----------|---------|
| `json` (default) | Full element/relationship detail. `fields` narrows the element and relationship rows only; `exclude` is what drops a whole visual array | Standard result with seven arrays: elements, relationships, visualMetadata, connections, groups, notes, images |
...
**Which containers the `tree` stats count.** `totalGroups` / `topLevelGroups` / `nestedGroups` count **both** kinds of box the group-layout family arranges: a native view group from `add-group-to-view`, **and** an ArchiMate `Grouping` element from `create-element` + `add-to-view`. Every such node carries `isGroup: true` — including nested ones — and every marked node is reachable by walking `children`, so the two readings agree at each scope:

- marked nodes at the **root** of `tree` = `topLevelGroups`
- marked nodes **anywhere** in `tree`, at any depth = `totalGroups`

`topLevelGroups` is the one that equals the `groupsPositioned` that `arrange-groups` will report for the same view — as long as no container is drawn inside a host, because these tools lay out the view's own children in canvas coordinates. Where one is, `arrange-groups` arranges it too, inside that host and reported in `nestedContainersArranged`; `optimize-group-order` and `adjust-view-spacing` neither count nor position it, and refuse such a view by naming what it holds rather than calling it groupless. An empty top-level container still counts, because it is still positioned — and a marked container always carries `childCount` and `children` even when empty, so you can read them off any marked node without checking which kind it is.

**If `totalGroups > 0` but `topLevelGroups == 0`**, every container on the view is nested inside something else. `layout-within-group` still applies to each of them. `arrange-groups` is not empty-handed either: a container whose only ancestors are hosts is the outermost one there is, so it arranges those inside their host, in the host's own coordinate space, and reports them in `nestedContainersArranged`. `optimize-group-order` and `adjust-view-spacing` have nothing to position and say so by naming the containers they can see, rather than reporting the view as having none. The `nextSteps` say which tool is which rather than recommending or dismissing all of them together.

Two counters deliberately do **not** follow that rule:

- `totalElements` counts every ArchiMate element on the view, a `Grouping` included — it stays one-for-one with `visualMetadata`.
- `ungroupedElements` counts only elements sitting **loose** on the canvas, so a top-level container is excluded: it is what the loose elements get placed into, not one of them.

Other element containers are **not** counted as groups. An `ApplicationComponent` holding `ApplicationFunction`s nests in the tree and can be passed to `layout-within-group`, but `arrange-groups` will not reposition **the host itself** — a host is not a zone — so it carries no `isGroup` marker and does not move the group counts. It does arrange the zones drawn inside it.

**All three formats agree on what a container is.** `summary` reports `Containers: N groups` counting both kinds (notes are reported separately, because an ArchiMate `Grouping` is a model concept rather than a visual annotation), and in `graph` every container node carries `isGroup: true` — the `Grouping` element nodes as well as the nodes tagged `_nodeType: "group"`. Changing `format` never changes how many containers a view is said to have.
...
>
> It lays out the **whole subtree**, not one level: a child of a top-level group that is itself a container (a nested group, or an ArchiMate element holding children) is sized from its contents with its title band reserved, then descended into — so a multi-level nesting needs no separate `layout-within-group` pass. `elementsRepositioned` counts every descendant moved, and each re-fitted nested container is reported in `nestedContainersFitted` with the rectangle it landed at, so you do not need a `get-view-contents` round trip to find out where the inner containers ended up.
...
**Quality target (recommended):** Use `targetRating` on `auto-layout-and-route` to automate the quality iteration loop. The tool internally runs `assess-layout` after each attempt and tunes whichever lever the worst-performing metric calls for — more spacing is one of several, not the whole loop. It stops as soon as **any** of these holds (at most 5 attempts):

| Stop | What to do about it |
|---|---|
| The target rating is reached | Nothing — you succeeded. |
| Every measured metric passes | Nothing further is measurable. |
| The worst metric is one no further iteration can move | Act on `suggestedRemediation`. Re-running with more spacing has already been ruled out. |
| Successive attempts stopped moving that metric | The lever is exhausted. Change the view's structure, not the parameters. |
| The attempt budget ran out | This one was cut short — a further run may still improve the result, **unless the run also regressed the view** (see below), in which case undo first. |

`terminationReason` in the response names the condition that stopped it. **Read it before reacting to `iterationsPerformed`:** a run that reports one iteration has usually measured that a second could not help, not given up early. This eliminates the need for manual assess → adjust → re-layout loops. Use `targetRating: "good"` for most views, or `"excellent"` for presentation-quality diagrams.

**The loop keeps the best attempt it PRODUCED — which can still be worse than the view you gave it.** The response discloses that rather than leaving you to find out, on every arm — the run is measured before it is dispatched, queued or proposed:

- `ratingBefore` is the overall rating the view held **before** the call, beside `achievedRating` for after. It is present on every run that used `targetRating`, including runs that improved.
- A `AUTO_LAYOUT_RATING_REGRESSED` entry in `structuredWarnings` fires whenever the applied result is worse than `ratingBefore`, naming every metric that moved and by how much. **Once the run has been applied, undo is the remedy and one undo is enough** — the whole run is committed as a single compound operation; a queued or awaiting-approval run has nothing to undo yet, and the warning names the remedy for that arm instead.

Note that meeting a target is *at least as good as*, not *better than before*: with `targetRating: "good"` an already-`excellent` view can be downgraded to `good`, satisfy the target and report `goal_reached_at_iteration_1`. The regression warning fires on that path too, which is why it is not gated on the target being missed. Do not read `achievedRating` alone as evidence the call helped — it describes the outcome, not the change.
...
**Full re-route — `auto-route-connections` (default `mode: "full"`):** only use if terminals-only leaves significant routing problems in the body of connections (e.g. genuine pass-throughs that ELK produced). **Always run `assess-layout` afterwards** — in dense hub-and-spoke topologies the obstacle-aware router may produce *more* crossings than ELK's simpler routing. If crossings increased, `undo` and either accept the residual non-orthogonal terminals (Tier 2R — they cap the routing tier at "fair") or increase element spacing and retry.
...
- **Annotation placement lands in corridors, and "place it after routing" does not prevent that.** After routing is exactly when there are corridors to land in. All three annotation tools — `add-note-to-view`, `add-view-reference-to-view`, `add-image-to-view` — therefore measure the rectangle they are about to place against the routes already on the view and return an `ANNOTATION_PLACED_IN_ROUTED_CORRIDOR` structured warning naming every connection it crosses, on the immediate, batched and approval arms alike. **The severity is not the same for the three.** A note is excluded from the assessor's layout node set, so a route through it is the informational `connectionThroughNoteCount` (caps at `good`); a view-reference and a standalone image are ordinary layout nodes, so a route through either is a **rated** `connectionPassThroughs` that can drive the view to `poor`. Move the annotation with `update-view-object` rather than re-routing around it — one rectangle is cheaper than the routes, and re-routing spends quality on a crossing that did not have to exist
- **Size hubs before the first route, not after.** `assess-layout` returns an `unsizedHubs` block naming every element whose connection fan-out exceeds its box, with the box it has and the box it needs. It answers on a view with no stored routing, which is the point: `hubPortQualityScore` is vacuously `1.0` before anything routes, so it cannot warn you then, and by the time it can, resizing the hub invalidates the routes already computed against the old perimeter. Sizing first is free; the retrofit costs a re-route and an undo
...
- **Workflow:** Import images with `add-image-to-model` first, then apply to view objects. Importing a set of icons? Pass `add-image-to-model` an `images` array to import them all in one call rather than one call per icon; the import tool is not available inside `bulk-mutate`, so import first and place afterwards. Set `imagePosition` and `showIcon` in the same `update-view-object` call to avoid intermediate states where both icons overlap.
...
- ⚠️ **A container is sized as a zone, not as an element — so this tool is safe to aim at a whole view built from zones.** An ArchiMate `Grouping` is **grown** to contain its children and is never shrunk nor sized to its own label; a childless one is left alone, and is disclosed in `skippedContainers` when the caller named it in `elementIds`. A native view group is never **collected as a target**, but the parent-fit cascade still **grows** one when a child this pass widened overflows it, and reports that in `resizedGroups`. To set a container's bounds yourself use `update-view-object`; to arrange what is inside it use `layout-within-group`.
...
| Existing view with truncated labels | `resize-elements-to-fit` on the view — `Grouping` zones are grown around their contents, never shrunk, so the whole view is a safe target |
...
- **Ignoring `structuredWarnings[]` from `auto-route-connections`:** the response carries a coded entry for every condition the router wants you to act on, and the code is what you key off — never the free-text `warnings[]`, which several unrelated producers write to. Read every entry, not just the one you recognise: `AUTO_NUDGE_SKIPPED_SIBLING_OVERLAP` (the autoNudge phase was a hard gate skip — re-running without first calling `layout-within-group` on the parent of `remediationViolatorIds` reproduces it), `AUTO_ROUTE_CROSSINGS_REGRESSED` (the re-route raised crossings; recover by whatever `remediationTool` names for the arm you are on — `undo` once applied, `end-batch` while queued, nothing to run while awaiting approval — then retry with `mode: "terminals-only"`), `EGRESS_LIFT_LAYOUT_BOUND` (a terminal hug is layout-bound; spread the elements, re-routing will not clear it), `CONNECTION_NOT_FOUND` (ids that did not resolve on the view), and `CONNECTION_ROUTED_THROUGH_NOTE` (a route was applied straight through a note — deliberate, since notes are not obstacles; move the **note** with `update-view-object` and re-assess, do not undo the route), and `AUTO_NUDGE_NET_ZERO` (an element `autoNudge` moved ended where it started, so it is absent from `nudgedElements` — the remedy is a spacing lever on the ids in `remediationViolatorIds`, never another route, because re-routing reproduces the same zero against the same geometry). The full list with actions is in `archimate://prompts/routing-preconditions-checklist`.
...
**Sizing:** Omit `height` and let the server fit it. `add-note-to-view` measures the note's wrapped text with real glyph metrics and sizes the box to it (floor 80, cap 600) — 80 is what short content resolves to, not a fixed default. The same fit runs on the **update** path: a later `update-view-object` (or `apply-positions`) that changes the note's `text` or `width` while omitting `height` re-fits the height to the new wrap, so a re-worded title cannot clip. Supply a `height` only to pin a fixed size — that is the only way a note ends up clipped, and it is what `assess-layout`'s `noteClipCount` flags. To repair one, re-send its `text` (or `width`) with `height` omitted; omitting `height` on its own is not a request the tool accepts, since at least one field must change.
...
The styling rail on `add-to-view` / `add-group-to-view` / `add-note-to-view` / `update-view-object` covers typography, line style, gradient, note border type, derived line colour, and outline opacity in addition to the basic colour / opacity / line-width fields. The connection tools — `add-connection-to-view` / `update-view-connection` / `auto-connect-view` — carry the colour, line-width and typography fields but **not** `lineStyle`; see *Connection line style* below.
...
- Applied via `IDiagramModelObject.setLineStyle(int)` (typed setter; LINE_STYLE_DEFAULT=-1, SOLID=0, DASHED=1, DOTTED=2, NONE=3).
- **Note:** this is a view-**object** property only. Connection line style is not controllable at all — see *Connection line style* below.
...
**A connection's line style cannot be set — use `lineColor` + `lineWidth` for view-level emphasis.**

An ArchiMate connection's dash pattern is determined entirely by its **relationship type**: a Flow renders dashed, a Realization dotted, a Serving solid, and so on. This is fixed in the renderer, not stored in the model — the connection figures hardcode their pattern per type, and the metamodel has no connection line-style attribute for a tool to write. Archi's own property panel and its Format Painter both deliberately exclude ArchiMate connections from the line-style control for the same reason.

Consequently `lineStyle` is **not a connection parameter**. Passing it to `add-connection-to-view`, `update-view-connection` or `auto-connect-view` is rejected with `INVALID_PARAMETER`. The rejection is deliberate: the parameter used to be accepted and then dropped in silence, which returned `success` for a change the model never received — and a caller that cannot see the canvas has no way to tell the difference.

**The supported idiom.** To carry a visual distinction across connections — synchronous versus asynchronous flows being the usual case — use `lineColor` together with `lineWidth`:

```
update-view-connection  viewConnectionId=<id>  lineColor="#D35400"  lineWidth=2   # async
update-view-connection  viewConnectionId=<id>  lineColor="#2C3E50"  lineWidth=1   # sync
```

Both fields are available on all three connection tools, so `auto-connect-view` can apply the pair to a whole filtered slice of relationships in a single call — filter by `relationshipTypes` (or `relationshipIds`) and style that slice, then call again for the next slice.

`lineStyle` remains fully supported on view **objects** — elements, groups and notes — via `add-to-view`, `add-group-to-view`, `add-note-to-view` and `update-view-object`.

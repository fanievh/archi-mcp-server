...
Two of these are also distributed as Claude Code slash commands, each embedding a **self-contained copy** of its prompt plus an argument-parsing preamble: `repo-to-archimate-model.md` as **`/repo-to-archi`** (adding a repository argument and a clone step), and `drawio-to-archimate-model.md` as **`/drawio-to-archi`** (taking the diagram source — a path, pasted mxGraph XML or a Confluence page — the layout mode, an optional companion document and optional icon packages). The command copies do not auto-update — `prompts/` is the source of truth, so re-sync after editing a prompt here. For **both** commands the sync is mechanical: run [`tools/sync-drawio-command.sh`](tools/sync-drawio-command.sh) or [`tools/sync-repo-command.sh`](tools/sync-repo-command.sh) to regenerate a copy, or `--check` to fail a build when it has drifted; CI gates both. `/repo-to-archi` additionally rewrites the prompt's `{{REPO_PATH}}`/`{{REPO_REF}}` placeholders into its `$ARGUMENTS` form, and its generator refuses to emit a copy in which any placeholder survived.
